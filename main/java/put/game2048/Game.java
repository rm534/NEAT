...
public class Game {
	...
	//Scuffed Functions
	public GameResult playSingle_agent(Agent agent, RandomDataGenerator random) {
		//Preconditions.checkNotNull(agentSupplier);
		Preconditions.checkNotNull(random);
	...
	public MultipleGamesResult playMultiple_agent(Agent agent, int numGames,
											RandomDataGenerator random) {
		//Preconditions.checkNotNull(agentSupplier);
		Preconditions.checkArgument(numGames > 0);
		Preconditions.checkNotNull(random);
		...
		MultipleGamesResult result = new MultipleGamesResult();
		for (int i = 0; i < numGames; ++i) {
			result.addGameResult(playSingle_agent(agent, random));
		}
		return result;
	}
